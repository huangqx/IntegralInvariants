function [Mesh] = read_from_obj(filename)
% Read a triangular mesh from a data file
nv = 0;
nf = 0;
vertex_pos = zeros(3, 300000);
face_vids = zeros(3, 600000);
...
while 1
    ...
    if tline(1) == 'f' && tline(2) == ' '
        nf = nf + 1;
        tp = sscanf(tline, 'f %d %d %d\n');
        face_vids(:, nf) = tp';
    end
end
fclose(f_id);

Mesh.vertexPoss = vertex_pos(:, 1:nv);
Mesh.faceVIds = face_vids(:, 1:nf);
...
Mesh.vertexPoss = single(Mesh.vertexPoss);
Mesh.faceVIds = uint32(Mesh.faceVIds);

% Remove redundant vertices
nv = size(Mesh.vertexPoss, 2);
flags = zeros(1, nv);
flags(Mesh.faceVIds) = 1;
ids = find(flags);
flags(ids) = 1:length(ids);
Mesh.vertexPoss = Mesh.vertexPoss(:, ids);
Mesh.faceVIds = flags(Mesh.faceVIds);
